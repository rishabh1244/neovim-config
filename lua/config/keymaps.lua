...
map("n", "<leader>vw", function()
  require("trouble").toggle({ mode = "diagnostics", filter = { severity = vim.diagnostic.severity.ERROR } })
end, { desc = "Trouble: show errors" })
...
map("n", "<C-j>", "<C-w>j")
map("n", "<C-k>", "<C-w>k")
map("n", "<C-l>", "<C-^>", { desc = "Previous active buffer" })
